...
public class DialogueController : MonoBehaviour
{
    ...
    [SerializeField] [Tooltip("Set this to the parent transform of where you want your dialogue boxes to spawn.")]
    private Transform dialogueBoxParent;

    [Header("Dialogue Settings")]
    [SerializeField]
    [Tooltip("This dictates how long dialogue boxes stay on screen. Lower/Higher to make them last longer/shorter")]
    private int wpmReadingSpeed;

    [SerializeField]
    [Tooltip("Dictates how fast text appears in the box. Use 0 if you wish for it to appear immediately.")]
    private float textTypeSpeed = 0.1f;
    ...
    [SerializeField]
    [Tooltip(
        "Lerp the texbox into place once it is called. This can be configured on the TextBox prefabs UiLerpElement component.")]
    ...
    public void NewDialogueInstance(string dialogue, string characterID)
    {
        ...
        {
            Debug.Log(
                "<color=cyan>Trying to reference a characterID, however there is no CharacterList referenced in your DialogueController.</color>");
            return;
        }
    ...
    private IEnumerator IterateQue()
    {
        ...
        if (lerpDialogueBoxesIn)
        {
            ...
            {
                Debug.Log(
                    "<color=cyan>Trying to lerp dialogue box, however we could not find UILerpElement.cs on it!</color>");
            }
        }
        ...
        {
            Debug.LogWarning(
                "<color=cyan>Your textTypeSpeed is too slow in comparison to your wpmReadingSpeed. Dialogue box will disappear before all text is shown.</color>");
        }
    ...
    {
        Debug.Log(ParseDialogueCustomStyle(
            "This is a test to find [TEST] and [/TEST] see [WHEENENNEN] how many times we can find it.[/WHEENENNEN]"));
    }
    ...
    public string ParseDialogueCustomStyle(string toParse)
    {
        ...
        if (rawString.Contains("["))
        {
            ...
            {
                Debug.Log(
                    "<color=cyan>Custom style tag detected in string, however DialogueController does not have a StyleSheetList referenced!</color>");
                return rawString;
            }
            ...
            foreach (Match match in Regex.Matches(rawString, pattern))
            {
                ...
                if (textStyle == null)
                {
                    Debug.Log("<color=cyan>Could not find the custom text style [" + matchedTag + "] in your text: " +
                              rawString + "</color>");
                    return rawString;
                }
                ...
                {
                    string sizeValue = textStyle.sizeChangeAsPercent
                        ? textStyle.fontSize + "%"
                ...
                {
                    taglessString = "<animate=" + textStyle.textAnimationSettings.GetSettingsSeed() + ">" +
                                    taglessString + "</animate>";
                }
